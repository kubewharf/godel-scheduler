...
func initReg() RegistryList {
	registry := RegistryList{
		...
		// GcRegistry uses aliyun registry to avoid pulling issue in China
		GcRegistry: "registry.aliyuncs.com/google_containers",
...
// GetE2EImage returns the fully qualified URI to an image (including version)
func GetDefaultE2EImage(image int) string {
	if len(imageConfigs[image].registry) > 0 {
		return fmt.Sprintf("%s/%s:%s", imageConfigs[image].registry,
			imageConfigs[image].name, imageConfigs[image].version)
	}
	return fmt.Sprintf("%s:%s", imageConfigs[image].name, imageConfigs[image].version)
}
...
// GetE2EImage returns the fully qualified URI to an image (including version)
func (i *Config) GetDefaultE2EImage() string {
	return fmt.Sprintf("%s:%s", i.name, i.version)
}

// GetPauseImageName returns the pause image name with proper version
func GetPauseImageName() string {
	return GetDefaultE2EImage(Pause)
}
