...
func AfterReadingAllFlags(t *TestContextType) {
	...
	if len(t.Host) == 0 && len(t.KubeConfig) == 0 {
		...
		if clusterConfig, err := restclient.InClusterConfig(); err == nil {
			if tempFile, err := ioutil.TempFile(os.TempDir(), "kubeconfig-"); err == nil {
				...
				klog.InfoS("Used a temporary kubeconfig file from in-cluster config", "config", tempFile.Name())
			}
		}
		if len(t.KubeConfig) == 0 {
			klog.InfoS("WARN: Failed to find in-cluster config, using default host", "defaultHost", defaultHost)
			t.Host = defaultHost
		}
	}
	...
	klog.InfoS("Tolerated taints when considering if nodes are ready", "taints", TestContext.NonblockingTaints)
	...
	if err != nil {
		if os.IsNotExist(errors.Cause(err)) {
			...
			klog.ErrorS(err, "Unknown provider", "provider", TestContext.Provider, "knownProviders", strings.Join(providers, " "))
		} else {
			klog.ErrorS(err, "Failed to setup provider config", "provider", TestContext.Provider)
		}
