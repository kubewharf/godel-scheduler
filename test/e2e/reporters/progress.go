...
func (reporter *ProgressReporter) postProgressToURL(b []byte) {
	...
	if err != nil {
		klog.InfoS("Failed to post progress update", "progressURL", reporter.progressURL, "err", err)
		return
	}
	if resp.StatusCode >= 400 {
		klog.InfoS("Unexpected response when posting progress update", "progressURL", reporter.progressURL, "statusCode", resp.StatusCode)
		if resp.Body != nil {
			defer resp.Body.Close()
			respBody, err := ioutil.ReadAll(resp.Body)
			if err != nil {
				klog.InfoS("Failed to read response body from posting progress", "err", err)
				return
			}
			klog.InfoS("Got the response body from posting progress update", "response", respBody, "err", err)
		}
