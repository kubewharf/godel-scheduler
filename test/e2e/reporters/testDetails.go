...
func NewDetailsReporterFile(filename string) *DetailsReporter {
	...
	if err != nil {
		klog.ErrorS(err, "Failed to get the absolute path", "file", filename)
		panic(err)
	}
	f, err := os.Create(absPath)
	if err != nil {
		klog.ErrorS(err, "Failed to create the file", "file", filename)
		panic(err)
	}
...
func (reporter *DetailsReporter) SpecDidComplete(specSummary *types.SpecSummary) {
	...
	if err != nil {
		klog.InfoS("Error in detail reporter", "err", err)
		return
	}
	_, err = reporter.Writer.Write(b)
	if err != nil {
		klog.InfoS("Failed to save test details in detail reporter", "err", err)
		return
	}
