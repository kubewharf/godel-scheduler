...
func (r *VolumeReactor) React(action core.Action) (handled bool, ret runtime.Object, err error) {
	...
	klog.V(4).InfoS("Entered React in VolumeReactor", "verb", action.GetVerb(), "resource", action.GetResource())
	...
	case action.Matches("create", "persistentvolumes"):
		...
		klog.V(4).InfoS("Created a PersistentVolume", "volumeName", volume.Name)
	...
	case action.Matches("create", "persistentvolumeclaims"):
		...
		r.changedSinceLastSync++
		klog.V(4).InfoS("Created a PersistentVolumeClaim", "claimName", claim.Name)
	...
	case action.Matches("update", "persistentvolumes"):
		...
		if found {
			...
			if reflect.DeepEqual(storedVolume, volume) {
				klog.V(4).InfoS("There were no updates in PersistentVolume", "volumeName", volume.Name)
				return true, volume, nil
			}
		...
		r.changedObjects = append(r.changedObjects, volume)
		r.changedSinceLastSync++
		klog.V(4).InfoS("Persisted the updated PersistentVolume", "volumeName", volume.Name)
		return true, volume, nil
	...
	case action.Matches("update", "persistentvolumeclaims"):
		...
		if found {
			...
			if reflect.DeepEqual(storedClaim, claim) {
				klog.V(4).InfoS("There were no updates in PersistentVolumeClaim", "claimName", claim.Name)
				return true, claim, nil
			}
			// Don't modify the existing object
		...
		r.changedObjects = append(r.changedObjects, claim)
		r.changedSinceLastSync++
		klog.V(4).InfoS("Persisted the updated PersistentVolumeClaim", "claimName", claim.Name)
		return true, claim, nil
	...
	case action.Matches("get", "persistentvolumes"):
		...
		if found {
			klog.V(4).InfoS("PersistentVolume found", "volumeName", volume.Name)
			return true, volume.DeepCopy(), nil
		}
		klog.V(4).InfoS("PersistentVolume not found", "volumeName", name)
	...
	case action.Matches("get", "persistentvolumeclaims"):
		...
		if found {
			klog.V(4).InfoS("PersistentVolumeClaim found", "claimName", claim.Name)
			return true, claim.DeepCopy(), nil
		}
		klog.V(4).InfoS("PersistentVolumeClaim not found", "claimName", name)
		return true, nil, apierrors.NewNotFound(action.GetResource().GroupResource(), name)

	case action.Matches("delete", "persistentvolumes"):
		name := action.(core.DeleteAction).GetName()
		klog.V(4).InfoS("Deleted a PersistentVolume", "volumeName", name)
	...
	case action.Matches("delete", "persistentvolumeclaims"):
		name := action.(core.DeleteAction).GetName()
		klog.V(4).InfoS("Deleted a PersistentVolumeClaim", "claimName", name)
...
func (r *VolumeReactor) injectReactError(action core.Action) error {
	...
	for i, expected := range r.errors {
		klog.V(4).InfoS("Started to match expected action and actual action",
			"expectedVerb", expected.Verb, "expectedResource", expected.Resource,
			"actualVerb", action.GetVerb(), "actualResource", action.GetResource())
		if action.Matches(expected.Verb, expected.Resource) {
			...
			klog.V(4).InfoS("Found matching error",
				"index", i, "expectedVerb", expected.Verb, "expectedResource", expected.Resource, "expectedError", expected.Error)
...
func (r *VolumeReactor) PopChange() interface{} {
	...
	for _, obj := range r.changedObjects {
		...
		case *v1.PersistentVolume:
			...
			klog.V(4).InfoS("Found a PersistentVolume in reactor queue", "volumeName", vol.Name)
		case *v1.PersistentVolumeClaim:
			claim, _ := obj.(*v1.PersistentVolumeClaim)
			klog.V(4).InfoS("Found a PersistentVolumeClaim in reactor queue", "claimName", claim.Name)
		}
	}
