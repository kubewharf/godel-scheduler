...
func (c *assumeCache) add(obj interface{}) {
	...
	if err != nil {
		klog.InfoS("Failed to get the meta namespace key when adding object to the assume cache", "err", err)
		return
	}
	...
	if objInfo, _ := c.getObjInfo(name); objInfo != nil {
		...
		if err != nil {
			klog.InfoS("Failed to get object version", "err", err)
			return
		}

		storedVersion, err := c.getObjVersion(name, objInfo.latestObj)
		if err != nil {
			klog.InfoS("Failed to get stored object version", "err", err)
			return
		}
		...
		if newVersion <= storedVersion {
			klog.V(6).InfoS("Skipped adding object to assume cache because the new version is not newer than the stored version",
				"entryDescription", c.description, "objectName", name, "newVersion", newVersion, "storedVersion", storedVersion)
			return
		}
	}
	...
	if err = c.store.Update(objInfo); err != nil {
		klog.InfoS("WARN: got error when updating stored object", "err", err)
	} else {
		klog.V(6).InfoS("Adding object to assume cache", "entryDescription", c.description, "objectName", name, "object", obj)
	}
}
...
func (c *assumeCache) delete(obj interface{}) {
	...
	name, err := cache.MetaNamespaceKeyFunc(obj)
	if err != nil {
		klog.InfoS("Failed to get the meta namespace key when deleting object from the assume cache", "err", err)
		return
	}
	...
	if err != nil {
		klog.InfoS("Failed to delete object from the assume cache", "entryDescription", c.description, "objectName", name, "err", err)
	}
}
...
func (c *assumeCache) List(indexObj interface{}) []interface{} {
	...
	if err != nil {
		klog.InfoS("Failed to list index", "err", err)
		return nil
	}
	...
	for _, obj := range objs {
		objInfo, ok := obj.(*objInfo)
		if !ok {
			klog.InfoS("Failed to list object", "err", &errWrongType{"objInfo", obj})
			continue
		}
...
func (c *assumeCache) Assume(obj interface{}) error {
	...
	klog.V(4).InfoS("Assumed object", "entryDescription", c.description, "objectName", name, "objectVersion", newVersion)
	return nil
}
...
func (c *assumeCache) Restore(objName string) {
	...
	if err != nil {
		...
		klog.InfoS("Failed to restore object", "entryDescription", c.description, "objectName", objName, "err", err)
	} else {
		objInfo.latestObj = objInfo.apiObj
		klog.V(4).InfoS("Restored object", "entryDescription", c.description, "objectName", objName)
	}
}
...
func (c *pvAssumeCache) ListPVs(storageClassName string) []*v1.PersistentVolume {
	...
	for _, obj := range objs {
		pv, ok := obj.(*v1.PersistentVolume)
		if !ok {
			klog.InfoS("Failed to list PVs", "err", &errWrongType{"v1.PersistentVolume", obj})
			continue
		}
		pvs = append(pvs, pv)
	}
