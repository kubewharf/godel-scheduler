...
func UpdateMovement(movementClient godelclient.Interface, movementLister v1alpha1.MovementLister, assumedPod *v1.Pod) bool {
	...
	for i := 0; i < MaxRetryAttemptsForMovement; i++ {
		err = func() error {
			...
			for _, ownerMovement := range newMovement.Status.OwnerMovements {
				if podutil.GetOwnerInfoKey(ownerMovement.Owner) != ownerInfoKey {
					continue
				}
				var allDesiredPodCount int
				for _, nodeSuggestion := range ownerMovement.NodeSuggestions {
				...
				if len(ownerMovement.TasksBreakSuggestion) >= allDesiredPodCount {
					break
				}
				breakSuggestion = true
				ownerMovement.TasksBreakSuggestion = append(ownerMovement.TasksBreakSuggestion, taskInfo)
				break
			}
