...
func New(_ runtime.Object, handle framework.SchedulerUnitFrameworkHandle) (framework.Plugin, error) {
	return &Rescheduling{handler: handle}, nil
}
...
func (i *Rescheduling) Locating(ctx context.Context, unit framework.ScheduleUnit, unitCycleState *framework.CycleState, nodeGroup framework.NodeGroup) (framework.NodeGroup, *framework.Status) {
	...
	for podOwner := range podOwners {
		suggestedMovementNodes := i.handler.GetSuggestedMovementAndNodes(podOwner)
		...
		for nodeName, suggestions := range suggestedMovementNodes {
			...
			for _, suggestion := range suggestions {
				...
				if nodeInfoHasDeletedPod(nodeInfo, i.handler.GetDeletedPodsFromMovement(movementName)) {
