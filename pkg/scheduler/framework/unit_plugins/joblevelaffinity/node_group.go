...
func divideNodesByPreferAffinity(
	...
	newNodeCircleElems := topologyTree[startIndexOfNewElem:]
	...
	parallelize.Until(parallelCtx, size, func(index int) {
		nc := newNodeCircleElems[index].nodeCircle
		...
		if hasRunningPods {
			...
			if preferAffinitySpecs != nil { // if those running pods are in different topologies, `preferAffinitySpecs` will be nil (refer to UT for getPreferAffinitySpecs)
				preferNodeCircles, err := groupNodesByAffinitySpecs(podLauncher, nc, assignedNodes, preferAffinitySpecs)
				if err != nil {
			...
			preferTopologyElems = getTopologyElems(preferNodeCircles)
			klog.V(4).InfoS("Started to sort node circles", "unitKey", unit.GetKey(), "sortRules", sortRules)
			sortAndMarkTopologyElems(parallelCtx, unit, preferTopologyElems, sortRules, request, newNodeCircleElems[index].cutOff)
		}
